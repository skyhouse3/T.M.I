							...
							<div class="postcontent nobottommargin">
								...
								<form class="nobottommargin" id="template-contactform"
									...
									<div class="col_full">
										...
										<textarea class="required sm-form-control"
											id="answer"
											name="template-contactform-message" rows="6" cols="30"><%if(s.getSanswer()!= null){%><%=s.getSanswer()%><%}else{%>작성된 내용이 없습니다.<%}%></textarea>
											
									</div>
									<input type="text" value="<%=s.getsNum()%>" style="display:none;" id="snum">
									...
									<div class="col_full">
										<button class="btn button nomargin" type="button"
											...
											name="template-contactform-submit" onclick="Answer('<%=s.getsNum()%>');">답변하기</button>
									</div>
								</form>
								<script>
									function Answer(no){
										...
										var snum = $('#snum').val();
										location.href="<%= contextPath %>/answer.sv?sNum=" + snum + "&answer=" + answer;
									}
								</script>
							</div>
						</div>
