<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"
	import="java.util.*, Board.model.*, Member.model.vo.*"%>
<%
	ArrayList<Boardman> list = (ArrayList<Boardman>)request.getAttribute("list");
					...
					<div class="tableArea">
						<table id="table">
							...
							<tr>
								<td><%=b.getRnum() %>
								<td><%=b.getMcode() %></td>
								<td><a href="<%=request.getContextPath()%>/bodetailman.bt?bnum=<%=b.getRnum()%>"><%=b.getRtitle() %></a></td>
								<td><%=b.getRrate() %></td>
								<td><%=b.getMrvDate() %></td>
								</tr>
								<%} %>
						...
						<div class="pagingArea" align="center">
							<!--  맨 처음으로 (<<) -->
							<a href="<%=contextPath%>/bomanagelist.bl?currentPage=1"
							...
							%>
							<a
								href="<%=contextPath%>/bomanagelist.bl?currentPage=<%=currentPage - 1%>"
								class="button button-light bgcolor-grey-light button-rounded button-reveal button-small button-3d tright"><i
							...
							%>
							<a href="<%=contextPath%>/bomanagelist.bl?currentPage=<%=maxPage%>"
								class="button button-light bgcolor-grey-light button-rounded button-reveal button-small button-3d tright"><i
