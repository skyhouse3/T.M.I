...
<%
	...
	ArrayList<MovieCharmingPoint> mcp = (ArrayList<MovieCharmingPoint>) request.getAttribute("mcplist");
	
	/* for(int i=0;i<;i++){ 
		mlist.get(i).setRate(mcp.get(i).getRrate());
	} */
...
<body>
			...
			<div class="row">
				...
				<div class="col-4 float-left movie-thumb-list">
					<div class="card">
						...
						<button type="button" class="btn btn-info" onclick="location.href='<%= request.getContextPath() %>/update.mo?mrid='+<%= b.getMcode() %>">수정</button>	
	...
	<script>
		...
		function UpdateMovie() {
			//console.log("실행 확인");
			location.href="<%=request.getContextPath()%>/views/movie/MovieUpdateForm.jsp";
		};
	</script>
