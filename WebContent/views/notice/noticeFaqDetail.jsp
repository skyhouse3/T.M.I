					...
					<div class="container clearfix">
						...
						<div class="postcontent nobottommargin">
							...
							<form class="nobottommargin" id="template-contactform"
								...
								<div class="col_one_third">
									<div class="left">
										<label>제목</label> <input type="text"
											id="template-contactform-name" value="<%=s.getStitle()%>"
									...
									<div class="left">
										<label for="template-contactform-name">작성자</label> <input
											...
											value="<%=s.getUserId()%>" class="sm-form-control required"
											readonly />
									</div>
								</div>
								...
								<div class="col_full">
									...
									<%
										if (s.getUserId().equals(mem)) {
									%>
									<%
										if (s.getStatus().equals("N")) {
									%>
									<textarea class="required sm-form-control" id="userContent"
										name="template-contactform-message" rows="6" cols="30"><%=s.getScontent()%></textarea>
									<%
										}
									%>
									<%
									...
									<textarea class="required sm-form-control" id="userContent"
										...
										readonly><%=s.getScontent()%></textarea>
									<%
										}
									%>
								</div>
								<div class="col_full">
									...
									<textarea class="required sm-form-control" id="answer"
										name="template-contactform-message" rows="6" cols="30"
										readonly><%=s.getSanswer()%></textarea>

								</div>
								<input type="text" value="<%=s.getsNum()%>"
									style="display: none;" id="snum">
								...
								<div id="btnplace">
									...
									<%
										if (mem.equals(s.getUserId())) {
									%>
									<button class="btn button nomargin" type="button"
										name="template-contactform-submit" onclick="updatef();">수정하기</button>
									<button class="btn button nomargin" type="button"
										name="template-contactform-submit" onclick="deletef();">삭제하기</button>
								</div>
								<input type="text" id="id" value="<%=mem%>"></input>
								<%
									}
								%>
								</form>
						...
						<script>
									function updatef(){
										...
										var snum = $('#snum').val();
										location.href="<%=contextPath%>/update.sv?sNum=" + snum + "&content=" + content;
									}
									function deletef(){
										var snum = $('#snum').val();
										var mem = $('#id').val();
										location.href="<%=contextPath%>/delete.sv?sNum="
										+ snum + "&mem=" + mem;
							}
						</script>
					</div>
